...
CREATE TABLE IF NOT EXISTS packages (
    ...
    max_products INT UNSIGNED DEFAULT 0,
    max_caris INT UNSIGNED DEFAULT 0,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
('sale.view', 'Satış görüntüleme'),
('sale.create', 'Satış oluşturma');

-- Link Admin role to all permissions
INSERT INTO role_permissions (role_id, permission_id)
SELECT roles.id, permissions.id FROM roles CROSS JOIN permissions WHERE roles.name = 'Admin';

-- Attach default admin user to Admin role
...
('Starter', 5, 500, 200),
('Pro', 25, 5000, 2000),
('Premium', 100, 20000, 8000);

-- Seed subscription for example company
